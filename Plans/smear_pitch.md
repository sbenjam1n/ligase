...
### Precedence / override with manual + modulated `smear_frequency` (GATE A)

**DEFAULT = OVERRIDE.** Placing the smear-pitch branch **after** the `smear_frequency_range` branch makes the note-derived Hz the **last writer** of the resonator frequency this block, so when `smear_pitch_control.enabled` it wins over both the range modulation and the manual `smear_frequency`. When **disabled** (default), the branch is skipped entirely and the stanza behaves exactly as today. The two-writer hazard on the resonator frequency is resolved by this **documented** ordering, never left as a silent race.

An optional **COMBINE** mode (smear pitch as the carrier note, `smear_frequency_range` sampled as a ±-semitone offset/vibrato: `hz = ref_hz · 2^((note + range_semitones)/12)`) is a clean extension since both still end at one `grain_smear_set_frequency` call — it is a **GATE A choice**, not the default.
...
- **COMBINE precedence mode** (range as a ±-semitone offset around the note) — documented as an opt-in extension, not built unless GATE A.1 elects it.
