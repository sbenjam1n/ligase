...
**Owner:** SLB
**Date:** 2026-06-24
**Status:** v1 COMPLETE — Steps 1-4 + 6 done (snapshot capture/recall of bands+scalars+discretes+scales+playable-FX shadows; IDW blend + surface + cursor; routes; **whole-surface persistence morph_save/morph_load**; manual docs). Headless-verified end to end (`tests/morph/MC1-5`). CV signal-inlet cursor DONE (morph_cursor 1, inlets 22/23). Natural-neighbour kernel DONE (mesh-free sampled Sibson, morph_interp 1). v1.1 essentially complete; the only remaining completeness item is FX scalar bases for distortion-enhancement/stut/bencina (their bands already morph). GATE-A decisions taken = the `[R]` recommendations (IDW v1 kernel behind a kernel-agnostic API; float `morph_x`/`morph_y` now, signal inlets v1.x; argmax discretes; shadow FX capture; stepper before `update_inlets`; full-param capture; caps 64).
