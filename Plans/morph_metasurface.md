...
**Owner:** SLB
**Date:** 2026-06-24
**Status:** IN PROGRESS — Step 1/GATE B + Step 2a + Step 2b-1 done. 2a = the 45 modulation-BAND capture/recall. 2b-1 = direct scalar bases (21: grain/pitch/smear-pitch/amplitude/pan/saw/sos/iot/quant) + discretes (30: modes/channels/enums) + the two pitch scale lists, headless-verified (`tests/morph/MC2_scalars.pd`: recall restores smear pitch 880Hz [scalar semitone + discrete source/enabled] and amplitude range 0.2/0.8). 2b-2 = FX shadow mirrors for the 11 playable-FX scalar bases (moog/smear/gdelay) -> **GATE C complete** (capture = bands + scalars + discretes + scales + FX shadows). Distortion-enhancement + stut/bencina scalar bases snap (documented completeness deferral; their bands morph). Next: Step 3 (GATE D — IDW blend + surface + cursor). GATE-A decisions taken = the `[R]` recommendations (IDW v1 kernel behind a kernel-agnostic API; float `morph_x`/`morph_y` now, signal inlets v1.x; argmax discretes; shadow FX capture; stepper before `update_inlets`; full-param capture; caps 64).
