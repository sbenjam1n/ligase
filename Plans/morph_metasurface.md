...
**Owner:** SLB
**Date:** 2026-06-24
**Status:** IN PROGRESS — Step 1/GATE B + Step 2a + Step 2b-1 done. 2a = the 45 modulation-BAND capture/recall. 2b-1 = direct scalar bases (21: grain/pitch/smear-pitch/amplitude/pan/saw/sos/iot/quant) + discretes (30: modes/channels/enums) + the two pitch scale lists, headless-verified (`tests/morph/MC2_scalars.pd`: recall restores smear pitch 880Hz [scalar semitone + discrete source/enabled] and amplitude range 0.2/0.8). 2b-2 = FX shadow mirrors for the 11 playable-FX scalar bases (moog/smear/gdelay) -> **GATE C complete** (capture = bands + scalars + discretes + scales + FX shadows). Distortion-enhancement + stut/bencina scalar bases snap (documented completeness deferral; their bands morph). Step 3/GATE D done: morph_apply_at (IDW blend — continuous weighted-sum, discrete argmax, exact-hit verbatim) + morph_point/morph_unplace/morph/morph_x/morph_y/morph_interp/morph_power. Headless-verified (`tests/morph/MC4_blend.pd`: morph 0.5 -> smear semitone 6=622.25Hz + amplitude band 0.4/0.9; exact hits reproduce). Selection tree (include/exclude) deferred to v1.1. Step 4/GATE E done: morph_route/morph_route_clear/morph_run/morph_stop/morph_pause + the per-block stepper in ligase_perform (before update_inlets). Headless-verified (`tests/morph/MC5_route.pd`: a 0.4s route swept smear pitch 440->880 Hz in 303 control-rate steps, halting at 880). Step 5 (CV signal inlets) = v1.1 per GATE A.4. Next: Step 6 (manual docs) -> v1 COMPLETE. GATE-A decisions taken = the `[R]` recommendations (IDW v1 kernel behind a kernel-agnostic API; float `morph_x`/`morph_y` now, signal inlets v1.x; argmax discretes; shadow FX capture; stepper before `update_inlets`; full-param capture; caps 64).
