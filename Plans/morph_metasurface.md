...
**Owner:** SLB
**Date:** 2026-06-24
**Status:** IN PROGRESS — Step 1/GATE B + Step 2a done. GATE B = module `src/morph.{c,h}` + types. Step 2a = the 45 modulation-BAND capture/recall (`snapshot`/`snapshot_recall`/`snapshot_clear`), headless-verified (`tests/morph/MC1_capture.pd`: snapshot→mutate→recall restores amplitude 0.2/0.8 + moog_cutoff 200/2000). Next: Step 2b (scalar bases + discretes + scale lists + the opaque-FX shadow mirrors) to complete GATE C; then Step 3 (blend/surface/cursor). GATE-A decisions taken = the `[R]` recommendations (IDW v1 kernel behind a kernel-agnostic API; float `morph_x`/`morph_y` now, signal inlets v1.x; argmax discretes; shadow FX capture; stepper before `update_inlets`; full-param capture; caps 64).
