...
Pitch-preserving granular delay based on the Ross Bencina "tapped delay line" architecture. Instead of a single read head, spawns up to 32 micro-grains inside the delay buffer. Each grain captures its OWN read position into the delay line — scattered behind the base delay by a random amount up to one grain length — and reads from there at unity playback rate. The per-grain position scatter is what makes it a diffuse granular CLOUD rather than a plain delay: overlapping grains read different points of the recent past instead of the identical delayed sample. (An earlier version gave every grain the same offset, so the sum collapsed to a plain delay with envelope ripple — that has been fixed.)
