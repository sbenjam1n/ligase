...
Wet level / saturation: the wet output is driven through a makeup gain (~6x) into a tanh soft-limit. The makeup sits well above unity to compensate for the position scatter, which sums grains incoherently and is therefore quieter than a coherent stack; tanh then bounds the result to ±1 (rounding the transient peaks) so it can never hard-clip, even when high feedback drives the buffer hot. Pushed hard, expect gentle analog-style saturation in the tails rather than digital clipping. The feedback path itself is NOT boosted or limited, so the recirculation loop gain stays = feedback (stable for feedback < 1).
